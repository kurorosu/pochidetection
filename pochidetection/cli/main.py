...
使用例 (-c で config を切り替えると RT-DETR / SSDLite が自動選択されます):
  ...
  ONNXエクスポート (モデルフォルダ内 config で自動判定, デフォルト FP32):
    uv run pochi export -m work_dirs/20260124_001/best
    uv run pochi export -m work_dirs/20260124_001/best --fp16

  TensorRTエクスポート (モデルフォルダ内 config で自動判定, デフォルト FP32):
    uv run pochi export-trt -i work_dirs/20260124_001/best/model_fp32.onnx
    uv run pochi export-trt -i work_dirs/20260124_001/best/model_fp32.onnx --fp16
        """,
    )
