...
class TestRTDetrOnnxBackendInit:
    """RTDetrOnnxBackend の初期化テスト."""

    def test_init_creates_session(self, rtdetr_onnx_path: Path) -> None:
    ...
    def test_init_file_not_found(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(FileNotFoundError, match="ONNXモデルが見つかりません"):
            RTDetrOnnxBackend(tmp_path / "nonexistent.onnx")
    ...
    def test_init_non_onnx_suffix_raises_value_error(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError, match=".onnx である必要があります"):
            RTDetrOnnxBackend(non_onnx)

    def test_init_stores_input_names(self, rtdetr_onnx_path: Path) -> None:
    ...
    def test_init_stores_output_names(self, rtdetr_onnx_path: Path) -> None:
...
class TestRTDetrOnnxBackendProviders:
    """Execution Providers のテスト."""

    def test_explicit_cpu_provider(self, rtdetr_onnx_path: Path) -> None:
...
class TestRTDetrOnnxBackendInfer:
    """RTDetrOnnxBackend.infer のテスト."""

    def test_infer_returns_torch_tensors(self, rtdetr_onnx_path: Path) -> None:
    ...
    def test_infer_output_shapes(self, rtdetr_onnx_path: Path) -> None:
    ...
    def test_infer_extra_keys_are_ignored(self, rtdetr_onnx_path: Path) -> None:
