...
class TestRTDetrOnnxExporterInit:
    """RTDetrOnnxExporter初期化のテスト."""
...
class TestRTDetrOnnxExporterExport:
    """RTDetrOnnxExporter.exportのテスト."""

    def test_export_creates_valid_file(self, rtdetr_onnx_path: Path) -> None:
        """エクスポートされたファイルが存在しサイズが正であることを確認する."""
        assert rtdetr_onnx_path.exists()
        assert rtdetr_onnx_path.stat().st_size > 0
...
class TestRTDetrOnnxExporterVerify:
    """RTDetrOnnxExporter.verifyのテスト."""
    ...
    def test_verify_returns_true(
        ...
        exporter = RTDetrOnnxExporter(model=rtdetr_model)
        assert exporter.verify(rtdetr_onnx_path, input_size=INPUT_SIZE) is True


class TestOnnxRuntimeInference:
    """ONNXモデルのONNX Runtime推論テスト."""

    def test_onnx_runtime_inference(self, rtdetr_onnx_path: Path) -> None:
        """ダミー入力でONNX Runtime推論が正常に完了し出力shapeが正しいことを確認する."""
        session = ort.InferenceSession(
            str(rtdetr_onnx_path), providers=["CPUExecutionProvider"]
        )
...
class TestRTDetrOnnxExporterLoadModel:
    """RTDetrOnnxExporter.load_modelのテスト."""
