...
### Changed
- plotter 系テストで class スコープ fixture と tmp_path_factory を導入し, HTML 生成を共有して `fig.write_html()` 呼び出し回数を削減. あわせて `test_rtdetr_model.py` の forward 入力を軽量化し, 実挙動検証を維持したまま実行時間を短縮. (N/A.)
